...
from local_constants import LT_IMAGE_PATH
...
class LocalLightTank(LocalTank, ABC):
    ...
    def shot_moves(self, target: tuple) -> tuple:
        ...
        fire_locs_around_enemy = LocalHex.possible_shots(target, LocalLightTank.__fire_deltas)
    ...
    def possible_shots(self) -> tuple:
        x, y, z = self._coord
        return tuple([(dx + x, dy + y, dz + z) for (dx, dy, dz) in LocalLightTank.__fire_deltas])
